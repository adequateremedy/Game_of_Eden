...
// ==========================================
// LEVEL 2: RESONATING MAGIC (Positive & Negative Synapses)
// ==========================================
...
function drawSynapse(ctx, x, y, radius, isPositive) {
    ...
    ctx.fillStyle = isPositive ? '#ff66b2' : '#8b4513';
    ctx.lineWidth = 2;
    ctx.shadowBlur = 10;
    ...
    // branching lines
    for (let i = 0; i < 4; i++) {
        let angle = (i * Math.PI / 2) + 0.5;
        let length = radius * 1.5;
        ctx.beginPath();
        ctx.moveTo(x, y);
...
function updateLvl3SimulationLogic() {
    // Adjusted expansion rates to be less punishing on later rounds
